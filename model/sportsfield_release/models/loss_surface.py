...
class LossSurfaceRegressor(BaseErrorModel):
    ...
    def create_model(self):
        ...
        assert self.feature_extractor.fc.weight.shape == (1, 512), "Son katman boyutu yanlış!"
        assert self.feature_extractor.fc.bias.shape == (1,), "Bias boyutu yanlış!"
    ...
    def forward(self, x):
        ...
        assert image_stack.shape[1] == self.input_features, f"Giriş kanal sayısı {image_stack.shape[1]}, beklenen {self.input_features}"
        
        # Feature extraction
        features = self.feature_extractor(image_stack)
        
        # Çıkış kontrolü
        assert features.shape[1] == self.out_dim, f"Çıkış boyutu {features.shape[1]}, beklenen {self.out_dim}"
        
        # Negatif değerleri engelle (sigmoid)
        if self.prevent_neg == 'sigmoid':
